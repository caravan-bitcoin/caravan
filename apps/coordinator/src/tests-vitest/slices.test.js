...
describe("slices utils", () => {
  ...
  describe("compareSlicesByPath", () => {
    it("should throw error if both slices are missing BIP32 path", () => {
      ...
      expect(() => sliceUtils.compareSlicesByPath(a, b)).toThrow(
        "Mising BIP32 path for comparing slices.",
      );
    });
  ...
  describe("compareSlicesByBalance", () => {
    it("should throw error if both slices are missing balance", () => {
      const a = { change: false };
      const b = { change: false };
      expect(() => sliceUtils.compareSlicesByBalance(a, b)).toThrow(
        "Missing balance for comparing slices",
      );
    });

    it("should compare slices by balance", () => {
      const a = {
        balanceSats: { isEqualTo: () => false, isGreaterThan: () => true },
      };
      const b = {
        balanceSats: { isEqualTo: () => false, isGreaterThan: () => false },
      };
    ...
    it("should return 0 for equal balances", () => {
      const a = {
        balanceSats: { isEqualTo: () => true, isGreaterThan: () => false },
      };
      const b = {
        balanceSats: { isEqualTo: () => true, isGreaterThan: () => false },
      };
  ...
  describe("compareSlicesByUTXOCount", () => {
    it("should throw error if both slices are missing utxos", () => {
      const a = { change: false };
      const b = { change: false };
      expect(() => sliceUtils.compareSlicesByUTXOCount(a, b)).toThrow(
        "Missing utxo array for comparing slices",
      );
    });
