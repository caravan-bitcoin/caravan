...
export default async function createTestWalletsAndFund(
  client: BitcoinCoreService,
) {
  ...
  const watcher_wallet = await client?.createWallet({
    wallet_name: walletNames[3],
