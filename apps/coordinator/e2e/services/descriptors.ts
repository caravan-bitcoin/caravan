 ...
 */
export async function extractWalletDescriptors(
  walletName: string,
  client: any,
  ...
  if (!targetDescriptor) {
    throw new Error(
      `No ${addressType} descriptor found for wallet: ${walletName}`,
    );
  }
  ...
  if (!xfp || !path) {
    throw new Error(
      `Invalid descriptors for wallet ${walletName}: xfp=${xfp}, path=${path}`,
    );
  }
...
export async function extractMultiWalletDescriptors(
  walletNames: string[],
  client: any,
  addressType: SingleSigAddressType,
): Promise<MultiWalletDescriptors> {
